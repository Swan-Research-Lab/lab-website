...
This system reads like a quietly editorial print magazine. The base canvas is off-white `{colors.canvas}` (#f5f5f5) holding warm near-black ink `{colors.ink}` (#0c0a09). The brand voltage is **photographic, not chromatic**: soft pastel atmospheric gradient orbs (mint, peach, lavender, sky, rose) drift through the page as the only "color" moments. There is no neon accent, no saturated CTA color, no dark-canvas atmosphere.
...
- **Canvas Deep** (`{colors.canvas-deep}` — #0c0a09): Same as ink — used for the rare dark hero band.
...
- Feature card grids: 2-up at desktop for hero splits, 3-up for content card grids.
...
**`top-nav`** — Background `{colors.canvas}`, text `{colors.ink}`, height 64px. Layout: lab wordmark left, primary horizontal menu (Research / People / Publications / News / Contact), optional "Join" or "Contact" primary CTA right.
...
**`content-card-stack`** — Stacked content preview cards (publications, projects, or news). Background `{colors.surface-card}`, rounded `{rounded.xl}`, no padding (children fill the card edge-to-edge).
...
- Lab-specific interactive components (member directory filters, publication search) only partially captured.
